...
class MapReduce(ABC):
    ...
    def __producer(self, data_arr):
        ...
        chunk = math.floor(len(data_arr) / len(self.worker_process))
        first = chunk + (len(data_arr) % len(self.worker_process))
        point = 0
        while first <= len(data_arr):
            message = {'array': data_arr[point: first]}
            zmq_socket.send_json(message)
            point = point + first if point == 0 else point + chunk
            first += chunk
...
if __name__ == '__main__':
    ...
    s = rand(-200, 200, 20)     # Start, End, List Length
    print(s)
    map = FindMax(4)
    map.start(s)
    map = FindSum(10)
